@include('shop.headeer')
  ...
  <div class="container">
    @if(count($datas))
    <h3><span class="label label-primary">Hasil pencarian dari : <b>{{$query}}</b></span></h3>
    <div class="row">
      ...
      <div class="col-sm-3 pro two" style="padding: 15px;float: left;">
        ...
        <form action="{{ url('/cart')}}" method="post">
          ...
          <div class="content-display">
            <a href="{{ url('detail/'.$data->id)}}"><h4 style="text-align:center;color:#4285F4;"><strong>{{ $data->name }}</strong></h4></a>
            <h5 style="text-align:center">Category</h5>
            <p style="text-align:justify;color:#a0a0a0;">
              {{ str_limit($data->description, 38) }}
              <hr class="hr">
            </p>
  ...
  @else
    <h3><span class="label label-danger">Oopss... Tidak ada hasil dari <b>{{$query}}</b></span></h3>
    @endif
</div>
